...
Feature: Sales API - User Operations
  ...
  Scenario: TC_Sales_API_User_002 - Restrict sale creation for regular users
    ...
    Then the API should return HTTP status code 403 or 400
    And the response body should indicate that the user is not authorized or request failed
  ...
  Scenario: TC_Sales_API_User_003 - Restrict sale deletion for regular users
    ...
    Then the API should return HTTP status code 403 or 404
    And the response body should indicate insufficient permissions or not found
