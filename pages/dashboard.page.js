...
class DashboardPage {
    constructor(page) {
        ...
        this.pageNumbers = page.locator('.pagination button, .pagination a');
        ...
        // Validation Messages
        this.validationError = page.locator('.error, .alert, .invalid-feedback, .text-danger');

        // No Results Message
        this.noResultsMessage = page.getByText(/No results found|No categories found|No data/i);
    }
    ...
    async clickPageNumber(pageNum) {
        const pageButton = this.page.locator(`.pagination button:has-text("${pageNum}"), .pagination a:has-text("${pageNum}")`);
    ...
    async isParentFilterVisible() {
        try {
            return await this.parentCategoryDropdown.isVisible();
        } catch {
            return false;
        }
    }
    ...
    async getPageNumbers() {
        const pageButtons = this.page.locator('.pagination button, .pagination a').filter({ hasNotText: 'Next' }).filter({ hasNotText: 'Previous' });
